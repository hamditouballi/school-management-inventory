...
<label class="block text-xs font-medium text-gray-700 mb-1">{{ __('messages.start_date') }}</label>
                    <input type="date" id="reportStartDate" class="w-full px-2 py-1 border rounded text-sm" value="{{ now()->startOfMonth()->format('Y-m-d') }}">
                </div>
                <div>
<label class="block text-xs font-medium text-gray-700 mb-1">{{ __('messages.end_date') }}</label>
                    <input type="date" id="reportEndDate" class="w-full px-2 py-1 border rounded text-sm" value="{{ now()->endOfMonth()->format('Y-m-d') }}">
                </div>
            </div>
            <button onclick="downloadConsumedMaterialsReport()" class="w-full px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 flex items-center justify-center">
...
<h4 class="font-semibold mb-2">{{ __('messages.department_consumption_report') }}</h4>
            ...
            <div class="space-y-2 mb-3">
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">{{ __('messages.start_date') }}</label>
                    <input type="date" id="deptReportStartDate" class="w-full px-2 py-1 border rounded text-sm" value="{{ now()->startOfYear()->format('Y-m-d') }}">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">{{ __('messages.end_date') }}</label>
                    <input type="date" id="deptReportEndDate" class="w-full px-2 py-1 border rounded text-sm" value="{{ now()->endOfYear()->format('Y-m-d') }}">
                </div>
                <div>
<label class="block text-xs font-medium text-gray-700 mb-1">{{ __('messages.items') }}</label>
                    <button onclick="showItemSelector()" class="w-full px-2 py-1 border rounded text-sm text-left hover:bg-gray-50">
<span id="selectedItemsText">{{ __('messages.all_items') }}</span>
                    </button>
                </div>
            </div>
            <button onclick="downloadDepartmentReport()" class="w-full px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 flex items-center justify-center">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                {{ __('messages.download_excel') }}
...
<h3 class="text-xl font-bold">{{ __('messages.select_items') }}</h3>
        ...
        <div class="mb-4">
            <button onclick="selectAllItems()" class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 text-sm mr-2">Tout sélectionner</button>
            <button onclick="deselectAllItems()" class="px-3 py-1 bg-gray-600 text-white rounded hover:bg-gray-700 text-sm">Tout désélectionner</button>
        </div>
        <div id="itemCheckboxList" class="space-y-2 mb-4 max-h-96 overflow-y-auto">
            <!-- Items will be loaded here -->
        </div>
        <div class="flex justify-end gap-2">
<button onclick="closeItemSelector()" class="px-4 py-2 border rounded hover:bg-gray-100">{{ __('messages.cancel') }}</button>
<button onclick="applyItemSelection()" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">{{ __('messages.save') }}</button>
        </div>
    </div>
</div>
...
document.getElementById('monthlySpending').textContent = '{{ __('messages.currency') }} ' + parseFloat(data.total_spent_this_month).toFixed(2);
        });

    // Monthly Consumption Chart
    fetch('/api/stats/consumption', { headers })
        .then(res => res.json())
        .then(data => {
    ...
    fetch('/api/stats/consumption-by-department', { headers })
        .then(res => res.json())
        .then(data => {
    ...
    fetch('/api/stats/spending', { headers })
        .then(res => res.json())
        .then(data => {
...
label: 'Amount Spent ({{ __('messages.currency') }})',
    ...
    // Top Items Chart
    fetch('/api/stats/top-items', { headers })
        .then(res => res.json())
        .then(data => {
    ...
    // Low Stock Table
    fetch('/api/stats/low-stock', { headers })
        .then(res => res.json())
        .then(data => {
            if (data.length === 0) {
                document.getElementById('lowStockTable').innerHTML = '<p class="text-gray-500">No low stock items</p>';
                return;
            }
let html = '<table class="min-w-full"><thead><tr class="bg-gray-50"><th class="px-4 py-2 text-left">{{ __('messages.designation') }}</th><th class="px-4 py-2 text-left">{{ __('messages.quantity') }}</th><th class="px-4 py-2 text-left">{{ __('messages.unit') }}</th></tr></thead><tbody>';
            data.forEach(item => {
                html += `<tr class="border-t"><td class="px-4 py-2">${item.designation}</td><td class="px-4 py-2 text-red-600 font-semibold">${item.quantity}</td><td class="px-4 py-2">${item.unit}</td></tr>`;
            });
            html += '</tbody></table>';
