...
<div class="bg-white rounded-lg shadow p-4 mb-6">
    <div class="grid grid-cols-3 gap-4">
        <div>
            ...
            <input type="text" id="searchInput" placeholder="{{ __('messages.search') }} by requester or department..." class="w-full px-3 py-2 border rounded" oninput="applyFilters()">
        </div>
        <div>
...
<div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <table class="min-w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('messages.requester') }}</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('messages.department') }}</th>
    ...
    <div class="px-6 py-4 border-t flex items-center justify-between">
        <div class="text-sm text-gray-700">
            Showing <span id="showingFrom">0</span> to <span id="showingTo">0</span> of <span id="totalRequests">0</span> requests
        </div>
...
<div id="createModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form id="createRequestForm" onsubmit="createRequest(event)">
            ...
            <button type="button" onclick="addItem()" class="mb-4 text-green-600 hover:text-indigo-800">+ Add Another Item</button>
            <div class="flex justify-end gap-2">
                <button type="button" onclick="closeCreateModal()" class="px-4 py-2 border rounded hover:bg-gray-100">{{ __('messages.cancel') }}</button>
                <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">{{ __('messages.create_request') }}</button>
            </div>
        </form>
    </div>
</div>
...
<div id="createModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
    <div class="bg-white rounded-lg p-6 w-full max-w-6xl max-h-[90vh] my-8 overflow-y-auto">
        ...
        <!-- Search bar -->
        <div class="mb-4">
            <input type="text" id="itemSearchInput" placeholder="{{ __('messages.search') }} items..." class="w-full px-4 py-2 border rounded-lg" oninput="filterItems()">
        </div>
        
        <!-- Items Grid -->
        <div id="itemsGrid" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6 max-h-96 overflow-y-auto">
        ...
        <div class="border-t pt-4">
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="closeCreateModal()" class="px-4 py-2 border rounded hover:bg-gray-100">{{ __('messages.cancel') }}</button>
                <button onclick="submitCartRequest()" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">{{ __('messages.create_request') }}</button>
            </div>
        </div>
    </div>
</div>
...
function createRequest(e) {
    ...
    for (let i = 0; i < itemCounter; i++) {
        ...
        if (itemId && quantity) {
            items.push({ item_id: parseInt(itemId), quantity_requested: parseFloat(quantity) });
        }
    }
...
function fulfillRequest(id) {
    if (confirm('Fulfill this request? This will decrease stock and generate Bon de Sortie.')) {
        fetch(`/api/requests/${id}/fulfill`, { method: 'POST', headers })
            .then(res => res.json())
            .then(data => {
                if (data.error) {
...
function updateCartDisplay() {
    ...
    cartContainer.innerHTML = Object.values(cart).map(({ item, quantity }) => `
