...
<div class="bg-white rounded-lg shadow p-4 mb-6">
    <div class="grid grid-cols-3 gap-4">
        <div>
            ...
            <input type="text" id="searchInput" placeholder="Search by supplier..." class="w-full px-3 py-2 border rounded" oninput="applyFilters()">
        </div>
        <div>
...
<div class="bg-white rounded-lg shadow overflow-hidden">
    ...
    <div class="px-6 py-4 border-t flex items-center justify-between">
        <div class="text-sm text-gray-700">
            Showing <span id="showingFrom">0</span> to <span id="showingTo">0</span> of <span id="totalPOs">0</span> purchase orders
        </div>
            ...
            <div class="space-y-4 mb-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="poSupplier" name="supplier" required class="w-full px-3 py-2 border rounded">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Date *</label>
                        <input type="date" id="poDate" name="date" required class="w-full px-3 py-2 border rounded">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2">Items *</label>
                    <div id="poItemsList" class="space-y-4"></div>
                    <button type="button" onclick="addPOItem()" class="mt-3 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 text-sm">+ Add Item</button>
                </div>
            </div>
            <div class="flex justify-end gap-2">
                <button type="button" onclick="closeCreateModal()" class="px-4 py-2 border rounded hover:bg-gray-100">Cancel</button>
                <button type="submit" id="submitBtn" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Create</button>
            </div>
        </form>
    </div>
</div>
...
function renderPOs() {
    ...
    if (pagePOs.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="px-6 py-4 text-center text-gray-500">No purchase orders found</td></tr>';
    } else {
...
function editPO(id) {
    fetch(`/api/purchase-orders/${id}`, { headers })
        .then(res => res.json())
        .then(po => {
            ...
            poItems.forEach((item, index) => {
                ...
                if (item.item_id) {
                    // Existing item
                    document.querySelector(`select[name="items[${index}][item_id]"]`).value = item.item_id;
                } else if (item.new_item_name) {
                    // New item
                    const radio = document.querySelector(`input[name="item_type_${index}"][value="new"]`);
                    radio.checked = true;
                    toggleItemType(index, 'new');
                    document.querySelector(`input[name="items[${index}][new_item_name]"]`).value = item.new_item_name;
                }
    ...
    itemDiv.innerHTML = `
        ...
        <div class="space-y-3">
            ...
            <div id="existing_item_${currentIndex}" class="space-y-2">
                ...
                <select name="items[${currentIndex}][item_id]" class="w-full px-3 py-2 border rounded text-sm">
                    <option value="">Select Item...</option>
                    ${allItems.map(item => `<option value="${item.id}">${item.designation} - {{ __('messages.currency') }} ${parseFloat(item.price).toFixed(2)}</option>`).join('')}
                </select>
            </div>
